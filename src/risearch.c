...
char *seq1_cli, *seq2_cli, *seq1file_name, *seq2file_name;
...
{
	int64_t len_seq1, len_seq2;
	char *one, *two;
	unsigned char *qseqIx, *tseqIx;
	fasta_t *ffpQ, *ffpT;	/*for query/target respectively */
	char *nameQ, *nameT;
	dsm_t dsm;
	int check;
	...
	if (seq2file_name) {	/* target given as file - or STDIN */
		ffpT = fasta_open(seq2file_name);
		if (ffpT == NULL) {
			fprintf(stderr, "Target file %s is not readable\n", seq2file_name);
			return -1;
		}
		while (fasta_read(ffpT, &two, &nameT, &len_seq2)) {
			count_q = 0;
			count_t++;
/*can be done already when reading in first place */
			...
			if (seq1file_name) {	/* query given as file */
				...
				while (fasta_read(ffpQ, &one, &nameQ, &len_seq1)) {
					count_q++;
...
void usage(char *progname)
{
	...
	fprintf(stderr, "  -t <FILE>, --target=FILE (.fa or .fa.gz)\n");
	fprintf(stderr, "     FASTA file for target sequence(s), use '-' for stdin\n");
...
void options(int argc, char *argv[])
{
	...
	int c, option_index;
	while ((c = getopt_long(argc, argv, "1vq:t:Q:T:d:M:N:K:y:z:m:s:e:n:w:l:f:p::", long_options, &option_index)) != -1)
		switch (c) {
		case 0:
			break;
		case '1':
