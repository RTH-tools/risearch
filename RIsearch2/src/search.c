...
//!!! not used !!!
saidx64_t binary_search(saidx64_t A[], saidx64_t key, saidx64_t imin, saidx64_t imax)
{
 ...
 */
saidx64_t sa_search_left(const saidx64_t *sa, saidx64_t start, saidx64_t end, saidx64_t offset, saint_t c)
{
	saidx64_t half;
 ...
 */
saidx64_t sa_search_interval(const saidx64_t *sa, saidx64_t start, saidx64_t end, saidx64_t offset, saidx64_t interval[6])
{
	int i;
	for(i = 0; i < 5; i++) {
	...
	while (i > 0 || j > 0) {
		...
		if (state == MA) {
			...
			if (Q(i) == T(j) && Q(i) != 5) //same nt, not 'N'
				strcat(temp_aln_str, "|");
			else if ( /*(noGUseed==0) & this would be necessary if you don't want to show G-Us with ":" */  ((Q(i) == 2 && T(j) == 1) || (Q(i) == 4 && T(j) == 3)))
			...
			if (Bq[TI(i,j)] == M[TI(i-1,j)] + (*S)[Q(i)][Q(i-1)][GAP][Tc(j)]) {
				state = MA;
				i--;
			} else if (Bq[TI(i,j)] == Bq[TI(i-1,j)] + (*S)[Q(i)][Q(i-1)][GAP][GAP]) {
				state = BQ;
				i--;
			}
		} else if (state == BT) {
			...
			if (Bt[TI(i,j)] == M[TI(i, j-1)] + (*S)[GAP][Q(i)][Tc(j)][Tc(j-1)]) {
				state = MA;
				j--;
			} else if (Bt[TI(i,j)] == Bt[TI(i,j-1)] + (*S)[GAP][GAP][Tc(j)][Tc(j-1)]) {
				state = BT;
				j--;
			}
		}
...
void print_result(char *qname, const saidx64_t *sa, const saidx64_t *qsa, aln_result_t *result)
{
	saidx64_t lpos, rpos;
	char strand;
...
void really_print_alignment(gzFile qout, aln_result_t *result) {
	...
	if (show_alignment == 1) {
		...
		gzprintf(qout, "%s\t%s\t%s\t%s\t%s\n", result->result_str, result->ia_str, result->target_str, result->temp_target_str, result->temp_ia_str);
	} else {
		gzprintf(qout, "%s\n", result->result_str);
	}
}
		...
		int *Bt_right)         //!< DP matrix for right extension, target bulged state
{
	...
	if (print_debug) fprintf(stderr, "analyzing q_seed:%lu-%lu, t_start: %lu \n ", q_start+1, q_start+q_seed_len, t_start);
	...
	if (noGUseed) {
		if (seed_flag) {
			if (q_start+1<seed[0] || q_start+q_seed_len>seed[1]) { 
				if (print_debug) {fprintf(stderr, "extension stopped due to start/end position for q_seed:%lu-%lu,  rem_query_len_left: %d, rem_query_len_right: %d, rem_target_len_left: %d, rem_target_len_right: %d \n", q_start+1, q_start+q_seed_len, rem_query_len_left, rem_query_len_right, rem_target_len_left, rem_target_len_right); }
				// extension stops due to start or end position
				return;
			}

			if( (rem_query_len_left-(seed[0]-1)>0 && rem_target_len_left>0 && pair_noGU[Q(-1)][T(-1)]) ||
				(rem_query_len_right-(query_len-seed[1])>0 && rem_target_len_right>0 && pair_noGU[Q(q_seed_len)][T(t_seed_len)]) ) {
				if (print_debug) {fprintf(stderr, "extension stopped due to maximality for q_seed:%lu-%lu,  rem_query_len_left: %d, rem_query_len_right: %d, rem_target_len_left: %d, rem_target_len_right: %d \n", q_start+1, q_start+q_seed_len, rem_query_len_left, rem_query_len_right, rem_target_len_left, rem_target_len_right); }
				// extension stops due to maximality within the seed
				return;
			}
		}
		else if((rem_query_len_left>0 && rem_target_len_left>0 && pair_noGU[Q(-1)][T(-1)]) || (rem_query_len_right>0 && rem_target_len_right>0 && pair_noGU[Q(q_seed_len)][T(t_seed_len)])) {
			if (print_debug) {fprintf(stderr, "stop extension whether maximality or str/end position! %lu %lu %d %d\n", q_start, t_start, q_seed_len, t_seed_len);}
			return;
		}
	}
	else{
		if (seed_flag) {
			...
			if( (rem_query_len_left-(seed[0]-1)>0 && rem_target_len_left>0 && pair[Q(-1)][T(-1)]) ||
				(rem_query_len_right-(query_len-seed[1])>0 && rem_target_len_right>0 && pair[Q(q_seed_len)][T(t_seed_len)]) ) {
				if (print_debug) {fprintf(stderr, "extension stopped due to maximality for q_seed:%lu-%lu,  rem_query_len_left: %d, rem_query_len_right: %d, rem_target_len_left: %d, rem_target_len_right: %d \n", q_start+1, q_start+q_seed_len, rem_query_len_left, rem_query_len_right, rem_target_len_left, rem_target_len_right); }
				// extension stops due to maximality within the seed
				return;
			}
		}
		else if((rem_query_len_left>0 && rem_target_len_left>0 && pair[Q(-1)][T(-1)]) || (rem_query_len_right>0 && rem_target_len_right>0 && pair[Q(q_seed_len)][T(t_seed_len)])) {
			if (print_debug) {fprintf(stderr, "stop extension whether maximality or str/end position! %lu %lu %d %d\n", q_start, t_start, q_seed_len, t_seed_len);}
			return;
		}
	}
...
#undef T
	...
	if (print_debug) {fprintf(stderr, "seed_score = seed_score : %d\n", total_score);}

	total_score += DP_left(M_left, Bq_left, Bt_left, rem_query_len_left, rem_target_len_left, qsa, sa, q_start, t_start, &best_left_i, &best_left_j);
	...
	total_score += DP_right(M_right, Bq_right, Bt_right, rem_query_len_right, rem_target_len_right, qsa, sa, q_start+q_seed_len-1, t_start + t_seed_len-1, &best_right_i, &best_right_j);

	if (print_debug) {fprintf(stderr, "total_score += DP_right : %d\n", total_score);}

	int nt_count = best_left_j + best_left_i + best_right_i + best_right_j + q_seed_len+ t_seed_len;

	if (print_debug) {
		fprintf(stderr, "extend_seed lq_left: %d lt_left: %d lq_right: %d lt_right: %d\n", rem_query_len_left, rem_target_len_left, rem_query_len_right, rem_target_len_right);
		fprintf(stderr, "nt_count is %d, using blj %li, bli %li, bri %li, brj %li, q_seedlen: %d, t_seedlen: %d \n", nt_count, best_left_j, best_left_i, best_right_i, best_right_j, q_seed_len, t_seed_len);
		fprintf(stderr, "**********************\n");
	}
...
void sa_evaluate_interval(sa_interval_list_t *results, query_t *query, const saidx64_t *sa, char strand)
{
	...
	int temp_subseed_score, temp_subseed_len, best_subseed_len, best_subseed_score;
...
# endif
	...
	if (show_alignment == 1) {
		...
		if (!result->query_str || !result->temp_query_str || !result->aln_str || !result->temp_aln_str || !result->target_str || !result->temp_target_str) {
		...
		result->ia_str = calloc(str_size, sizeof(char));
		result->temp_ia_str = calloc(str_size, sizeof(char));
		if (!result->query_str || !result->temp_query_str || !result->aln_str || !result->temp_aln_str || !result->target_str || !result->temp_target_str || !result->ia_str || !result->temp_ia_str) {
			fprintf(stderr, "Failed allocating result attributes\n");
			exit(EXIT_FAILURE);
		}
	}
...
#define T(ix) (comp[XRIS(sa[t_start+(ix)])])
		...
		best_subseed_len=seed_len; best_subseed_score=seed_score; best_subseed_spos=0;
		if (seed_threshold_flag)
		{
...
#undef T
		...
		for(j = sl; j < sr; j++) {
			...
			for (k = ql; k < qr; k++) {
				...
				extend_seed(j, k, sa, qsa, result, seed_score, query->length, seed_len, best_subseed_spos, best_subseed_len, best_subseed_score, query->name, query->seed, M_left, Bq_left, Bt_left, M_right, Bq_right, Bt_right);
		...
		sa_interval_list_t **results)  //!< List of intervals with seed matches
{
	...
	if(depth <= offset) {
		// MISMATCH UPDATE
		if ((mismatch_count==0) || ((mismatch_count>0) && (mismatch_count<=seed_mismatch[0]) && (last_match_count >= seed_mismatch[1]) && (last_match_count < depth))){
			sa_interval_add(results, sl, sr, ql, qr, offset);
		}
	}

	if(!sa_search_interval(qsa, ql, qr, offset, qint) || !sa_search_interval(sa, sl, sr, offset, sint))
		return;
	...
	*/
	...
	if(qint[1]-qint[0]) {
		if (sint[1]-sint[0])
			sa_parallel_match_neg(qsa, qint[0], qint[1], sa, sint[0], sint[1], offset, depth, last_match_count+1, mismatch_count, results);
	}
	/* c */
	if (qint[2] - qint[1]) { 
		if (sint[2]-sint[1])
			sa_parallel_match_neg(qsa, qint[1], qint[2], sa, sint[1], sint[2], offset, depth, last_match_count+1, mismatch_count, results);
	}
	/*  g */
	if (qint[3]-qint[2]) {
		if(sint[3]-sint[2])
			sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[2], sint[3], offset, depth, last_match_count+1, mismatch_count, results);
		/* wobble pair */
		/* g - comp('a') We need to match the complement of U: A */
		if(noGUseed==0)
			if(sint[1]-sint[0])
				sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[0], sint[1], offset, depth, last_match_count+1, mismatch_count, results);
	}

	/* u */
	if (qint[5]-qint[4]) {
		if(sint[5]-sint[4])
			sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[4], sint[5], offset, depth, last_match_count+1, mismatch_count, results);
		/* wobble pair */
		/* source should be C because we're matching the complementary strand */
		if(noGUseed==0)
			if(sint[2]-sint[1])
				sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[1], sint[2], offset, depth, last_match_count+1, mismatch_count, results);
	}
	...
	if (seed_mismatch_flag) {
		...
		if ( (mismatch_count < seed_mismatch[0]) && (offset>seed_mismatch[1]) &&(last_match_count < depth) ) {
			/* MISMATCH for a*/
			if(qint[1]-qint[0]) {
				if (sint[2]-sint[1])
					sa_parallel_match_neg(qsa, qint[0], qint[1], sa, sint[1], sint[2], offset, depth, 0, mismatch_count+1, results);
				if (sint[3]-sint[2])
					sa_parallel_match_neg(qsa, qint[0], qint[1], sa, sint[2], sint[3], offset, depth, 0, mismatch_count+1, results);
				if (sint[5]-sint[4])
					sa_parallel_match_neg(qsa, qint[0], qint[1], sa, sint[4], sint[5], offset, depth, 0, mismatch_count+1, results);
			}
			/* MISMATCH for c*/
			if (qint[2] - qint[1]) {
				if (sint[1]-sint[0])
					sa_parallel_match_neg(qsa, qint[1], qint[2], sa, sint[0], sint[1], offset, depth, 0, mismatch_count+1, results);
				if (sint[3]-sint[2])
					sa_parallel_match_neg(qsa, qint[1], qint[2], sa, sint[2], sint[3], offset, depth, 0, mismatch_count+1, results);
				if (sint[5]-sint[4])
					sa_parallel_match_neg(qsa, qint[1], qint[2], sa, sint[4], sint[5], offset, depth, 0, mismatch_count+1, results);
			}
	
			/* MISMATCH for g*/
			if (qint[3]-qint[2]) {
				if (sint[2]-sint[1])
					sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[1], sint[2], offset, depth, 0, mismatch_count+1, results);
				if (sint[5]-sint[4])
					sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[4], sint[5], offset, depth, 0, mismatch_count+1, results);
				if (noGUseed)
					if(sint[1]-sint[0])
						sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[0], sint[1], offset, depth, 0, mismatch_count+1, results);
			...
			if (qint[5]-qint[4]) {
				if (sint[1]-sint[0])
					sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[0], sint[1], offset, depth, 0, mismatch_count+1, results);
				if (sint[3]-sint[2])
					sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[2], sint[3], offset, depth, 0, mismatch_count+1, results);
				if (noGUseed)
					if(sint[2]-sint[1])
						sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[1], sint[2], offset, depth, 0, mismatch_count+1, results);
